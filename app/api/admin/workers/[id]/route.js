...
export async function PATCH(request, context) {
  try {
    ...
    await new Promise((resolve) => {
      db.run("ALTER TABLE workers ADD COLUMN auto_lock_exempt INTEGER DEFAULT 0", (err) => {
        if (err && !isDuplicateColumnError(err)) {
          console.error("Add workers.auto_lock_exempt failed:", err);
        }
        resolve();
      });
    });
    ...
    const existing = await new Promise((resolve, reject) => {
      db.get(
        "SELECT first_name, last_name, email, phone_number, status, status_locked, verified, auto_lock_exempt FROM workers WHERE id = ?",
        [id],
        (err, row) => (err ? reject(err) : resolve(row ?? null))
      );
    });
    ...
    let finalAutoLockExempt = existing.auto_lock_exempt ? 1 : 0;
    if (status !== existing.status) {
      finalLock = 1; // Auto-lock if status changed by admin
      finalAutoLockExempt = 0;
    }
    if (status_locked !== undefined) {
      finalLock = status_locked ? 1 : 0;
      finalAutoLockExempt = status_locked ? 0 : 1;
    }
    ...
    if (new_password != null && String(new_password).trim() !== "") {
      ...
      let query = "UPDATE workers SET first_name = ?, last_name = ?, email = ?, phone_number = ?, status = ?, status_locked = ?, auto_lock_exempt = ?, verified = ?, password = ?, updated_at = CURRENT_TIMESTAMP";
      let qParams = [first_name, last_name, email, phone_number, status, finalLock, finalAutoLockExempt, finalVerified, hashedPassword];
      ...
      let query = "UPDATE workers SET first_name = ?, last_name = ?, email = ?, phone_number = ?, status = ?, status_locked = ?, auto_lock_exempt = ?, verified = ?, updated_at = CURRENT_TIMESTAMP";
      let qParams = [first_name, last_name, email, phone_number, status, finalLock, finalAutoLockExempt, finalVerified];
    ...
    if ((existing.auto_lock_exempt ? 1 : 0) !== finalAutoLockExempt) changes.push("auto_lock_exempt");
