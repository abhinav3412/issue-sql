...
export async function POST(request) {
    try {
        ...
        await new Promise((resolve) => {
            db.run("ALTER TABLE workers ADD COLUMN auto_lock_exempt INTEGER DEFAULT 0", (err) => resolve());
        });
        ...
        if (requestDetails && requestDetails.assigned_worker) {
            ...
            if (recentRatings.length >= 10) {
                ...
                if (avg < 3) {
                    ...
                    await new Promise((resolve, reject) => {
                        db.run(
                            "UPDATE workers SET status = 'Offline', status_locked = 1, auto_lock_exempt = 0, lock_reason = 'Low Rating' WHERE id = ?",
                            [workerId],
                            (err) => (err ? reject(err) : resolve())
                        );
                    });
